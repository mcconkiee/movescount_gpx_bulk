...
const route = {
  ...
  create: function(filePath, done) {
    ...
    parseGpx(filePath).then(track => {
      ...
      if (trackpoints.length >= 1) {
        geocoding.location(
          { latitude: trackpoints[0].lat, longitude: trackpoints[0].lon },
    ...
    var req = http.request(options, function(res) {
      ...
      res.on('end', function() {
        ...
        if (json && json.route) {
          ...
          move.create(
            { routeData: postData.route, helperData: helperData },
            done
          );
        } else {
          done(null, {});
  ...
  removeRoute: function(id, done) {
    ...
    var options = {
      method: 'DELETE',
      url: 'http://www.movescount.com/Move/RouteAppData',
      headers: {
        'cache-control': 'no-cache',
        cookie: this.cookie
      },
    ...
    request(options, function(error, response, body) {
      const json = JSON.parse(body);
      console.log('removed', json, error);
      done(error, json);
    });
  }
};
